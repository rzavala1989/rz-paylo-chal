...
const FormApplicant: React.FC = () => {
  ...
  const { firstName, lastName, email, phone, numDependents, dependents } =
    values;
            ...
            {({ push, remove }) => (
              <div>
                {values.dependents.map(
                  (dependent: Dependent, index: unknown) => (
                    <div
                      key={index}
                    ...
                    >
                      ...
                      <Field
                        className={
                          touched?.dependents?.[index]?.firstName &&
                          errors?.dependents?.[index]?.firstName
                            ? 'shadow appearance-none border rounded w-full py-2 my-1 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline border-red-500'
                      ...
                      <Field
                        className={
                          touched?.dependents?.[index]?.lastName &&
